...
{	
    ...
    while (QueueSize(q) > 0) {
        // Dequeue the front BSTree node in the queue and set address
        BSTLink address = QueueDequeue(q);
		// Print the value of the front BSTree node in the queue
        showBSTreeNode(address);
		// If left and right child exists, add to queue in this specific order
        if (address->left != NULL) {
			QueueEnqueue(q, address->left);
		}
		if (address->right != NULL) {
			QueueEnqueue(q, address->right);
		}
    }
...
int BSTreeNumLeaves(BSTree t)
{
	...
	//if the only node have two NULL leaf, then return node is leaf node and return 1
	else if ((t->left == NULL) && (t->right == NULL)){
		return 1;
	}
	else if ((t->left != NULL)||(t->right != NULL)){
		leaf += BSTreeNumLeaves(t->right);
		leaf += BSTreeNumLeaves(t->left);
	}
	return leaf;
}
